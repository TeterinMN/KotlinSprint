...
fun main() {
    ...
    for (i in listOfIngredients)
        println("Ингредиент: ${listOfIngredients.indexOf(i) + 1} - $i")
    print("Какой ингредиент вы хотите заменить? >> ")
    val inputUser = readln()
    if (inputUser in listOfIngredients) {
        print("На какой ингредиент заменить $inputUser >> ")
        val newIngredient = readln()
        val temp = listOfIngredients.indexOf(inputUser)
        listOfIngredients[temp] = newIngredient
        for (i in listOfIngredients)
            println("Ингредиент: ${listOfIngredients.indexOf(i) + 1} - $i")
    } else {
